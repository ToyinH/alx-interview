...
function getMovieCharacters (movieId) {
  return new Promise((resolve, reject) => {
    ...
    request.get(movieUrl, (error, response, body) => {
      if (error) {
        reject(new Error(`Error fetching movie details: ${error}`));
        return;
      }

      if (response.statusCode !== 200) {
        reject(new Error(`Failed to fetch movie details: ${response.statusCode}`));
        return;
      }
      ...
      charactersUrls.forEach((url) => {
        charactersPromises.push(new Promise((resolveCharacter, rejectCharacter) => {
          request.get(url, (errorCharacter, responseCharacter, bodyCharacter) => {
            if (errorCharacter) {
              rejectCharacter(new Error(`Error fetching character details: ${errorCharacter}`));
              return;
            }

            if (responseCharacter.statusCode !== 200) {
              rejectCharacter(new Error(`Failed to fetch character details: ${responseCharacter.statusCode}`));
              return;
            }
